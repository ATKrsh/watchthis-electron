...
export const FloatingPopupPlayer: React.FC<FloatingPopupPlayerProps> = ({
  ...
  return (
    <div
      ...
      className={`fixed z-[150] rounded-2xl overflow-hidden shadow-2xl select-none group cursor-move bg-black ${
        isNeon 
          ? 'border border-accent-cyan/50 shadow-[0_0_18px_rgba(0,240,255,0.25),inset_0_0_1px_rgba(0,240,255,0.4)] ring-1 ring-accent/30' 
          : 'border border-white/25 shadow-[0_0_16px_rgba(255,255,255,0.1)] ring-1 ring-white/10'
    ...
    >
      ...
      >
        ...
        >
          ...
          <div className="flex items-center gap-1 bg-black/75 backdrop-blur-md px-2 py-1 rounded-xl border border-white/15">
            <Move className="w-3 h-3 text-slate-400 cursor-move" />
            <button
              onClick={cycleSpeed}
              title={isPdf ? "Cycle Auto Page-Flip Rate (1x-5x)" : "Cycle Speed (1x-5x)"}
              className="px-1.5 py-0.2 rounded bg-white/10 hover:bg-accent hover:text-white text-[9px] font-mono text-accent-cyan font-light transition-colors"
            >
              {speed}x
            </button>
          </div>
          ...
          <div className="flex items-center gap-1 bg-black/75 backdrop-blur-md px-1.5 py-1 rounded-xl border border-white/15">
            {/* Snap-Back */}
            <button
        ...
        <div className="absolute inset-0 bg-black/25 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center pointer-events-none">
          <div className="w-9 h-9 rounded-full bg-accent/90 text-white flex items-center justify-center shadow-lg">
        ...
        <div 
          onClick={(e) => e.stopPropagation()}
          className="absolute bottom-2 left-2 right-2 px-2.5 py-1 rounded-xl bg-black/75 backdrop-blur-md border border-white/15 flex items-center justify-between opacity-0 group-hover:opacity-100 transition-opacity duration-200 text-xs z-30"
        >
