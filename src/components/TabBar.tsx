...
export const TabBar: React.FC = () => {
  ...
  const getTabIcon = (tab: TabItem) => {
    ...
    return <Layers className="w-3.5 h-3.5 text-accent-cyan flex-shrink-0" />;
  };
  ...
  return (
    <nav aria-label="Media tabs" className={`h-11 px-4 flex items-center border-b select-none font-sans transition-all z-30 ${
      isNeon 
        ? 'bg-[#060912]/95 border-white/[0.08]' 
        : 'bg-[#0a0d17]/95 border-white/[0.06]'
    }`}>
      {/* Tab Strip Container */}
      <div 
        ref={tabListRef}
        className="flex items-center gap-2 overflow-x-auto no-scrollbar flex-1 py-1 pr-2"
      ...
      >
        {tabs.map((tab) => {
          ...
          return (
            <div
              ...
              title={tab.title}
              className={`group relative flex items-center gap-2.5 h-8 px-3.5 rounded-xl border text-xs cursor-pointer transition-all max-w-[240px] min-w-[130px] flex-shrink-0 ${
                isActive
                  ? isNeon
                    ? 'bg-surface-elevated/95 text-white border-accent-cyan/50 shadow-[0_0_15px_rgba(0,240,255,0.2)] font-medium'
                    : 'bg-[#151a28] text-white border-white/25 font-medium shadow-sm'
                  : 'bg-white/[0.03] hover:bg-white/[0.07] text-slate-400 hover:text-slate-200 border-white/[0.05] hover:border-white/15 font-normal'
              }`}
            >
              {/* Active neon bottom indicator line */}
              {isActive && (
                <div className={`absolute bottom-0 left-3 right-3 h-[2px] rounded-full ${
                  isNeon ? 'bg-gradient-to-r from-accent-cyan to-accent-neon shadow-[0_0_10px_#00f0ff]' : 'bg-white/90'
                }`} />
              )}
        ...
        <button
          ...
          className="flex items-center justify-center w-8 h-8 rounded-xl bg-white/[0.04] hover:bg-accent/25 hover:text-accent-cyan text-slate-400 border border-white/[0.08] hover:border-accent/40 transition-all flex-shrink-0 shadow-sm"
      ...
      <div className="flex items-center gap-1 pl-2.5 border-l border-white/[0.08] flex-shrink-0">
        <button
          type="button"
