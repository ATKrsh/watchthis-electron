...
// Concurrency pools: 4 parallel video workers, 2 parallel PDF workers (safe within Chromium hardware limits)
const MAX_VIDEO_CONCURRENT = 4;
 ...
 * Uses high-concurrency throttling (4 workers), persistent IndexedDB caching,
 * and smart fallback seeking to guarantee lightning-fast indexing without GPU decoder starvations.
 */
  ...
  const promise = (async () => {
    ...
    return new Promise<string>((resolve) => {
      const executeTask = () => {
        ...
        // 1.8-second safety timeout per video (prevents queue stalls)
 ...
 * Runs on a dedicated PDF worker queue with IndexedDB persistence and guaranteed memory cleanup.
 */
        ...
        try {
          ...
          if (!doc) {
            doc = await Promise.race([
              ...
              new Promise<never>((_, rej) => setTimeout(() => rej(new Error('PDF load timeout')), 5000))
            ]);
          }
          ...
          // Zero-Leak: Explicitly destroy PDF document to free Web Worker heap
