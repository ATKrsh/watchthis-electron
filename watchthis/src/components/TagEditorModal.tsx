      ...
      <div className={`w-full max-w-md p-5 border shadow-2xl space-y-4 animate-in fade-in zoom-in-95 duration-150 ${
        ...
        <div className="flex items-center justify-between border-b border-white/[0.1] pb-3">
          <div className="flex items-center gap-2.5">
            <div className="w-8 h-8 bg-accent-magenta/20 border border-accent-magenta/50 flex items-center justify-center text-accent-magenta">
              <Tag className="w-4 h-4" />
            </div>
            <div>
              <h3 className="text-sm font-bold text-white">Add & Edit Video Tags</h3>
        ...
        <div>
          <label className="text-[10px] font-mono uppercase tracking-wider text-slate-400 font-bold block mb-1.5">
            Auto-Extracted Tags ({video.smartTags.length})
          </label>
          <div className="flex flex-wrap gap-1.5 max-h-24 overflow-y-auto pr-1">
            {video.smartTags.length === 0 ? (
              ...
              video.smartTags.map(tag => (
                <span
                  key={tag}
                  className="px-2 py-0.5 text-xs font-mono bg-white/5 border border-white/10 text-slate-300 font-medium"
                >
                  #{tag}
                </span>
              ))
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-1.5 min-h-[40px] p-2 bg-black/50 border border-white/[0.1] mb-3">
            {customTags.length === 0 ? (
              ...
              customTags.map(tag => (
                <span
                  key={tag}
                  className="inline-flex items-center gap-1 px-2 py-0.5 text-xs font-mono bg-accent-magenta/20 border border-accent-magenta/40 text-accent-magenta font-semibold"
                >
                  #{tag}
                  <button
          ...
          <div className="flex gap-1.5">
            ...
            <button
              ...
              className="px-3 h-8 bg-white/10 hover:bg-white/20 border border-white/15 text-white text-xs font-semibold flex items-center gap-1 transition-all"
            >
        ...
        {/* Actions Footer */}
        <div className="flex items-center justify-end gap-2 pt-2 border-t border-white/[0.1]">
          <button
            onClick={onClose}
            className="px-3.5 h-8 border border-white/10 text-xs font-mono text-slate-400 hover:text-white hover:bg-white/5 transition-all"
          >
            Cancel
          </button>
          <button
            onClick={handleSave}
            className="flex items-center gap-1.5 px-4 h-8 bg-accent hover:bg-accent-hover text-white text-xs font-mono font-bold shadow-md transition-all"
          >
